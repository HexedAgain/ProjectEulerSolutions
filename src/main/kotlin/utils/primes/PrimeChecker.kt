...
class PrimeChecker(private val maxSmallPrime: Int = 1000) {
    ...
    fun checkPrime(n: Long): Boolean {
        if (maxSmallPrime < sqrt(n.toDouble()).toInt()) throw IllegalStateException(
            "You passed n = $n, but maxSmallPrime is $maxSmallPrime. Set maxSmallPrime to at least sqrt($n) = ${sqrt(n.toDouble()).toInt()}"
        )
        if (n <= 1) return false
